...
def entropy_on_union(cur_dir, now, opt, model, device, current_res, current_win, win_size, max_item=0, ent='entropy'):
    # R' = R U R^{new}
    uni_x = current_res[0] + current_win[0]
    uni_y = current_res[1] + current_win[1]
    uni_user = current_res[2] + current_win[2]
    ...
    with torch.no_grad():
        if ent == 'entropy':
            pro = forward_entropy(model, uni_loader, device, max_item_id= max(max(max(current_win[0])), max(current_win[1])))
        elif ent == 'cross':
            pro = forward_cross_entropy(model, uni_loader, device, max_item_id=max(max(max(current_win[0])), max(current_win[1])))
        elif ent == 'wass':
            pro = forward_wass(model, uni_loader, device, max_item_id=max(max(max(uni_x)), max(uni_y)))
...
def fix_new_entropy_on_union(cur_dir, now, opt, model, device, current_res, current_win, win_size, max_item, max_user,
    ...
    if left_win_size > 0:
        sampled_old = entropy_on_union(cur_dir, now, opt, model, device, current_res, left_win, left_win_size,
                                       max_item=max_item, ent=ent)
        return sampled_x + sampled_old[0], sampled_y + sampled_old[1], sampled_user + sampled_old[2]
    else:
